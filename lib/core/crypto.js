 ...
 * @module dsh-remote-workspaces/core/crypto
 */
import {
  createCipheriv,
  createDecipheriv,
...
const AAD = Buffer.from('dsh-remote-workspaces/link/v1')
...
export function authKey(clusterKey) {
  return hkdf(clusterKey, 'dsh-remote-workspaces/cluster-auth', 'handshake-mac/v1')
}
...
function deriveKeys(secrets, salt, clusterKey, initiatorId, responderId) {
  ...
  return {
    clientToServer: hkdf(ikm, salt, 'dsh-remote-workspaces/link/v1/c2s'),
    serverToClient: hkdf(ikm, salt, 'dsh-remote-workspaces/link/v1/s2c'),
    transcriptHash: hkdf(ikm, salt, `dsh-remote-workspaces/transcript/v1/${initiatorId}>${responderId}`),
  }
}
